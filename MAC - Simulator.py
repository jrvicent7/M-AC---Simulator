...
def plot_V_curve():
    ...
    def plotar_grafico_unico():
        ...
        # GRÁFICO 2: If x Ia
        ax2.plot(If, abs(Ia), linewidth=2, label=f'Máq. {contador_de_motores} | Pin = {Pot_acionada/746:.1f} HP | If = {If_inicial} A')
        ...
        if flag_grafico_1 == 0: # É o primeiro gráfico?
            ...
            if msinc_values[-1][1] != Pot_acionada or msinc_values[-1][0]!= If_inicial:
                ...
                if not pot_mudou:  # potência acionada atual é igual a anterior e só If mudou
                    ...
                    # Curva V
                    ax2.plot(If, abs(Ia), linewidth=2, label=f'Máq. {contador_de_motores} | Pin = {Pot_acionada/746:.1f} HP | If = {If_atual} A')
                    ax2.set_xlabel('Corrente de Campo, A', fontsize=12, fontweight='bold')
                    ax2.set_ylabel('Corrente de Armadura, A', fontsize=12, fontweight='bold')
                    ax2.set_xticks(np.arange(int(min(If)) - 1.5, int(max(If)) + 1.5, 0.25))
                    ax2.set_yticks(range(int(abs(Ia_fp_unitario)) - 1, int(max(abs(Ia))) + 1, 1))
                ...
                # if msinc_values[-1][1] != Pot_acionada and msinc_values[-1][0] == If_inicial:

                #     nova_Pin = Pot_acionada + msinc_values[-1][2] + msinc_values[-1][3]
                #     # Ea mantém o módulo (If igual), apenas o ângulo de carga muda
                #     novo_angulo_de_carga = math.asin((nova_Pin * msinc_values[-1][4])/(3 * msinc_values[-1][7] * abs(msinc_values[-1][6])))

                #     nova_Ea = cmath.rect(abs(msinc_values[-1][6]), -novo_angulo_de_carga)
                #     novo_angulo_Ea = math.degrees(cmath.phase(nova_Ea))

                #     # ✅ delta1 recalculado com base no novo ângulo de carga, não o do motor inicial
                #     delta1_novo = novo_angulo_de_carga  # já está em radianos

                #     nova_Ia = (msinc_values[-1][7] - nova_Ea) / (1j*msinc_values[-1][4])
                #     novo_angulo_Ia = math.degrees(cmath.phase(nova_Ia))

                #     nova_XsIa = 1j*msinc_values[-1][4] * nova_Ia
                #     novo_angulo_XsIa = math.degrees(cmath.phase(nova_XsIa))
                ...
                #     #Calcula o valor de If quando FP = 1
                #     If_FP_unitario = (abs(Ea_FP_unitario)*If_atual)/abs(nova_Ea) # Relação - Ea1 / If
                ...
                #     Ea_2 = (If*abs(nova_Ea)) / If_atual # Calcula cada valor que Ea pode assumir dentro da variação de If (de If_min a If_max)

                #     delta2 = np.arcsin(abs(nova_Ea) / abs(Ea_2) * math.sin(delta1_novo)) # Calcula todos os Ângulos de Carga para cada valor de Ea_2 (de If_min a If_max)
                    
                #     Ea = Ea_2 * np.exp(1j * -delta2) # Cria uma array com todos os fasores Ea (de If_min a If_max)

                #     Ia = (Vfase_atual-Ea) / (1j*Xs_atual) # Cria uma array com todos os fasores Ia (de If_min a If_max)

                #     msinc_values.append([msinc_values[-1][0], nova_Pin, msinc_values[-1][2], msinc_values[-1][3], msinc_values[-1][4], nova_Ia, nova_Ea, msinc_values[-1][7]])

                #     # GRÁFICO 2: If x Ia
                #     ax2.plot(If, abs(Ia), 'r-', linewidth=2, label='Corrente de Armadura')
                #     ax2.set_xlabel('Corrente de Campo, A', fontsize=12, fontweight='bold')
                ...
                #     print(magnitud_Ia_fp_unitario)
                #     print(f"Módulo: {abs(Ia_fp_unitario):.4f}")
                #     print(f"Ângulo: {math.degrees(cmath.phase(Ia_fp_unitario)):.4f}°")


                # elif msinc_values[-1][1] == Pot_acionada and msinc_values[-1][0] != If_inicial:
                ...
                #     #Calcula o valor de If quando FP = 1
                #     If_FP_unitario = (abs(Ea_FP_unitario)*If_anterior)/abs(Ea_anterior) # Relação - Ea1 / If
                ...
                #     Ea_2 = (If*abs(Ea_anterior)) / If_anterior # Calcula cada valor que Ea pode assumir dentro da variação de If (de If_min a If_max)

                #     delta2 = np.arcsin(abs(Ea_anterior) / abs(Ea_2) * math.sin(delta1)) # Calcula todos os Ângulos de Carga para cada valor de Ea_2 (de If_min a If_max)
                ...
                #     # Marcar pontos importantes no gráfico de corrente
                #     ax2.plot(If_FP_unitario, abs(Ia_fp_unitario), 'go', markersize=8, label=f'FP = 1; Ia = {abs(Ia_fp_unitario):.2f} A')
                #     ax2.plot(If_atual, abs(Ia1), 'mo', markersize=8, label=f'If = {If_atual}; Ia = {abs(Ia1):.2f} A')
                #     ax2.legend()
                ...
                # # Fasor tensão de Fase
                # # ax3.arrow(0, 0, msinc_values[0][7], 0,
                # #         head_width=5, head_length=6,
                # #         fc='green', ec='green',
                # #         lw=2.5,
                # #         length_includes_head=True, alpha=0.7,
                # #         label=f'Vφ {Vfase:.2f} ∠{0}°')
